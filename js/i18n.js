...
(function () {
    ...
    function currentLang() {
        ...
        let saved = null;
        try { saved = window.localStorage.getItem('yoshigen-lang'); } catch (_) { /* Storage may be blocked in local previews. */ }
        return LANGS.includes(param) ? param : (LANGS.includes(saved) ? saved : 'ja');
    }

    function renderRows(target, rows) {
        if (!target || !Array.isArray(rows)) return;
        target.innerHTML = rows.map((row) => `<dl class="info-row"><dt>${escapeHTML(row.label)}</dt><dd>${escapeHTML(row.value)}</dd></dl>`).join('');
    }

    function renderFeatureCards(target, items, linkLabel) {
        if (!target || !Array.isArray(items)) return;
        target.innerHTML = items.map((item, index) => `<article class="feature-card tilt-card"><span class="feature-index">${String(index + 1).padStart(2, '0')}</span><div><h3>${escapeHTML(item.title)}</h3><p>${escapeHTML(item.text)}</p></div>${item.link ? `<a class="card-link" href="${escapeHTML(item.link)}">${escapeHTML(linkLabel || 'View more')}</a>` : ''}</article>`).join('');
    }
    ...
    function renderFlow(target, items) {
        if (!target || !Array.isArray(items)) return;
        target.innerHTML = items.map((item) => `<article class="flow-card"><span class="flow-step">${escapeHTML(item.step)}</span><h3>${escapeHTML(item.title)}</h3><p>${escapeHTML(item.text)}</p></article>`).join('');
    }

    function renderProducts(target, items, images, copy) {
        if (!target || !Array.isArray(items)) return;
        target.innerHTML = items.map((item, index) => `<article class="product-card" data-group="${escapeHTML(item.group)}"><div class="product-visual"><img src="${escapeHTML(images[item.id] || '')}" alt="${escapeHTML(item.title)} — ${escapeHTML(copy.illustrationLabel)}" width="420" height="320" loading="lazy"><span class="product-number">${String(index + 1).padStart(2, '0')}</span></div><div class="product-card-content"><span class="category-pill">${escapeHTML(item.category)}</span><h3>${escapeHTML(item.title)}</h3><p>${escapeHTML(item.text)}</p><div class="product-examples"><small>${escapeHTML(copy.examplesLabel)}</small><p>${escapeHTML(item.examples)}</p></div><a class="product-enquiry" href="contact.html">${escapeHTML(copy.enquiryLabel)} <span aria-hidden="true">↗</span></a></div></article>`).join('');
    }

    function renderProductFilters(copy) {
        const target = document.querySelector('[data-render="productFilters"]');
        if (!target) return;
        target.innerHTML = copy.filters.map((filter, index) => `<button type="button" data-filter="${escapeHTML(filter.id)}" aria-pressed="${index === 0}">${escapeHTML(filter.label)}</button>`).join('');
        target.onclick = (event) => {
            const button = event.target.closest('button[data-filter]');
            if (!button) return;
            target.querySelectorAll('button').forEach((node) => node.setAttribute('aria-pressed', String(node === button)));
            document.querySelectorAll('[data-render="products"] .product-card').forEach((card) => {
    ...
    function renderGallery(target, items, images) {
        if (!target || !Array.isArray(items)) return;
        target.innerHTML = items.map((item) => `<article class="gallery-card tilt-card"><div class="gallery-visual" style="background-image:url('${escapeHTML(images[item.id] || '')}')"></div><div class="gallery-card-content"><h3>${escapeHTML(item.title)}</h3><p>${escapeHTML(item.text)}</p></div></article>`).join('');
    }
    ...
    function renderDynamic(data, site) {
        ...
        renderProducts(document.querySelector('[data-render="products"]'), data.products && data.products.items, media.products, data.products);
        renderProducts(document.querySelector('[data-render="homeProducts"]'), data.products && data.products.items, media.products, data.products);
    ...
    async function applyLanguage(lang) {
        ...
        try { window.localStorage.setItem('yoshigen-lang', lang); } catch (_) { /* Language switching does not require storage. */ }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.querySelectorAll('.lang-switch button').forEach((button) => {
            button.addEventListener('click', () => applyLanguage(button.dataset.lang).catch(showLoadError));
        });
    });
})();
